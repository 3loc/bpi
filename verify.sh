...
# Check 2e: node --test gate over every extension's test suites.
...
# adding a build step or a new dependency. Test discovery is derived
# from extensions/ (any *.test.ts), so new suites are covered
# automatically. Skipped on Node < 22.6 (strip-types was experimental
# before then); absence of node is a warn, not a fail — the gate is a
# strict improvement, not a baseline.
if command -v node >/dev/null 2>&1; then
	...
	if (( node_major >= 23 )) || (( node_major == 22 && node_minor >= 6 )); then
		...
		echo "warn: node $node_major.$node_minor < 22.6 — skipping extension test gate"
	fi
else
	echo "warn: node not installed — skipping extension test gate"
fi
