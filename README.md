...
| [`extensions/escape-think-tags`](extensions/escape-think-tags/index.ts) | `pi --escape-think-tags` — opt-in render-time markdown transformer that wraps literal think-tag sequences in assistant text as inline code spans (defensive: prevents downstream markdown consumers that interpret those sequences as thinking-block markers from dropping or hiding parts of the prose; does not touch real thinking blocks, user messages, tool calls, or the on-disk session JSONL) |
